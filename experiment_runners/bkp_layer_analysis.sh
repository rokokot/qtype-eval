...
for LANGUAGE in "${LANGUAGES[@]}"; do                           # 1. Main tasks (question_type and complexity)

    for LAYER in "${LAYERS[@]}"; do
        mkdir -p "${OUTPUT_BASE_DIR}/${LANGUAGE}/layer_${LAYER}"
        
        run_standard_experiment "$LANGUAGE" "$LAYER" "question_type" "classification"                   # Run question_type experiments (classification)

        
        run_standard_experiment "$LANGUAGE" "$LAYER" "complexity" "regression"                          # Run complexity experiments (regression)
    done
done

                                                                 
for LANGUAGE in "${LANGUAGES[@]}"; do   # 2. Submetric tasks
    for LAYER in "${LAYERS[@]}"; do
