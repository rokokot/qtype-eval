...
def process_dataset():
    ...
    if args.add_ablation:
        ...
        for feature in linguistic_features:
            ...
            tydi_abl_path = os.path.join(args.tydi_dir, "ablation", f"tydi_train_ablation_no_{feature}.csv")
            ...
            ud_abl_path = os.path.join(args.ud_dir, "ablation", f"ud_test_ablation_no_{feature}.csv")
            ...
            # Add dev ablation if dev directory is provided
            if args.dev_dir:
                
                dev_abl_path = os.path.join(args.dev_dir, f"dev_ablation_no_{feature}.csv")
                if os.path.exists(dev_abl_path):
                    print(f"Processing dev ablation dataset for {feature}")
                    dev_abl = pd.read_csv(dev_abl_path)
                    if 'source' in dev_df.columns:
                      dev_df = dev_df.drop('source', axis=1)
            
                    if 'dataset_source' not in dev_abl.columns:
                        dev_abl['dataset_source'] = 'dev'
                    dev_abl = dev_abl.sample(frac=1, random_state=args.random_seed).reset_index(drop=True)
                    
                    split_name = f"dev_ablation_no_{feature_name}"
                    dev_abl_csv_path = f"qtcV2_upload_pack/{split_name}.csv"
                    dev_abl.to_csv(dev_abl_csv_path, index=False)
                    
                    datasets_info[split_name] = {
                        "path": dev_abl_csv_path,
                        "num_examples": len(dev_abl)
                    }
