...
def visualize_feature_space(df, output_dir, method="tsne", n_samples=5000):
    ...
    if len(df) > n_samples:
        sampled_df = df.groupby("language").apply(
            lambda x: x.sample(min(len(x), n_samples // len(LANGUAGES)), random_state=42)
    ...
    scatter = plt.scatter(
        sampled_df["x"], 
        sampled_df["y"], 
        c=sampled_df["lang_norm_complexity_score"], 
        cmap="viridis", 
        alpha=0.8, 
        s=50
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split", type=str, default="train", choices=["train", "validation", "test"], help="Dataset split to visualize")
    parser.add_argument("--cache-dir", type=str, default="./data/cache", help="Cache directory for datasets")
    parser.add_argument("--dim-reduction", type=str, default="tsne", choices=["tsne", "umap"], help="Dimensionality reduction method")
    parser.add_argument("--n-samples", type=int, default=5000, help="Number of samples to use for feature space visualization")
