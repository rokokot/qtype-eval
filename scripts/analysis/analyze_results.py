...
def generate_question_type_plots(df: pd.DataFrame, output_dir: str):
    ...
    sns.barplot(data=task_df, x="language_name", y="test_accuracy", hue="model_type", palette="viridis")
    ...
    if "test_f1" in task_df.columns:
        plt.figure(figsize=(14, 8))
        sns.barplot(data=task_df, x="language_name", y="test_f1", hue="model_type", palette="viridis")
...
def generate_complexity_plots(df: pd.DataFrame, output_dir: str):
    ...
    if "test_r2" in task_df.columns:
        plt.figure(figsize=(14, 8))
        sns.barplot(data=task_df, x="language_name", y="test_r2", hue="model_type", palette="viridis")
    ...
    if "test_mse" in task_df.columns:
        plt.figure(figsize=(14, 8))
        sns.barplot(data=task_df, x="language_name", y="test_mse", hue="model_type", palette="viridis")
...
def generate_submetric_plots(df: pd.DataFrame, output_dir: str):
    ...
    if not lm_submetric_df.empty and "test_r2" in lm_submetric_df.columns:
        ...
        sns.barplot(data=lm_submetric_df, x="submetric", y="test_r2", hue="language_name", palette="viridis")

        plt.title("Submetric R² Scores for LM Probe across Languages", fontsize=16)
    ...
    if "test_r2" in submetric_df.columns:
        ...
        plt.figure(figsize=(14, 8))
        sns.barplot(data=avg_results, x="submetric", y="test_r2", hue="model_type", palette="viridis")
...
def generate_cross_lingual_plots(df: pd.DataFrame, output_dir: str):
    ...
    cross_lingual_df = df[df["cross_lingual"]]
    ...
    for task in cross_lingual_df["task"].unique():
        ...
        if task == "question_type" and "test_accuracy" in task_df.columns:
            ...
            pivot_df = task_df.pivot_table(
                values="test_accuracy", index="language", columns="eval_language", aggfunc="mean"
            )

            # Plot heatmap
            plt.figure(figsize=(12, 10))

            sns.heatmap(pivot_df, annot=True, fmt=".3f", cmap="viridis", vmin=0, vmax=1, cbar_kws={"label": "Accuracy"})

            plt.title("Cross-Lingual Question Type Accuracy (Train → Evaluate)", fontsize=16)
        ...
        # For complexity task - R2 heatmap
            ...
            # Create pivot table
            pivot_df = task_df.pivot_table(values="test_r2", index="language", columns="eval_language", aggfunc="mean")
            ...
            plt.title("Cross-Lingual Complexity R² Score (Train → Evaluate)", fontsize=16)
            plt.xlabel("Evaluation Language", fontsize=14)
            plt.ylabel("Training Language", fontsize=14)
            plt.tight_layout()
...
def generate_summary_tables(df: pd.DataFrame, output_dir: str):
    ...
    if not question_type_df.empty and "test_accuracy" in question_type_df.columns:
        ...
        accuracy_table = pd.pivot_table(
            question_type_df, values="test_accuracy", index="model_type", columns="language", aggfunc="mean"
        )
        ...
        if "test_f1" in question_type_df.columns:
            f1_table = pd.pivot_table(
                question_type_df, values="test_f1", index="model_type", columns="language", aggfunc="mean"
            )
    ...
    if not complexity_df.empty and "test_r2" in complexity_df.columns:
        ...
        r2_table = pd.pivot_table(
            complexity_df, values="test_r2", index="model_type", columns="language", aggfunc="mean"
        )
        ...
        if "test_mse" in complexity_df.columns:
            mse_table = pd.pivot_table(
                complexity_df, values="test_mse", index="model_type", columns="language", aggfunc="mean"
            )
    ...
    if not submetric_df.empty and "test_r2" in submetric_df.columns:
        ...
        submetric_table = pd.pivot_table(
            submetric_df, values="test_r2", index="model_type", columns="submetric", aggfunc="mean"
        )
    ...
    cross_lingual_df = real_df[real_df["cross_lingual"]]
    ...
    if not cross_lingual_df.empty:
        ...
        if not q_type_cl_df.empty and "test_accuracy" in q_type_cl_df.columns:
            cl_accuracy_table = pd.pivot_table(
                q_type_cl_df, values="test_accuracy", index="language", columns="eval_language", aggfunc="mean"
            )
        ...
        if not complexity_cl_df.empty and "test_r2" in complexity_cl_df.columns:
            cl_r2_table = pd.pivot_table(
                complexity_cl_df, values="test_r2", index="language", columns="eval_language", aggfunc="mean"
            )
