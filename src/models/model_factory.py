    ...
    if model_type == "dummy":
        if task_type == "classification":
            return DummyClassifier(
                strategy=kwargs.get("strategy", "most_frequent"), random_state=kwargs.get("random_state", 42)
            )
        else:
        ...
        if task_type != "classification":
            logger.warning("Logistic regression is for classification only; continuing anyway")
        ...
        if task_type != "regression":
            logger.warning("Ridge regression is for regression only; continuing anyway")

        return Ridge(alpha=kwargs.get("alpha", 1.0), random_state=kwargs.get("random_state", 42))
        ...
        raise ValueError(f"Unk type: {model_type}")
